...
@Get('refund/approvedRefunds/getAll')
 ...
 // @UseGuards(JwtGuard)
  ...
  async markAsDeparted(@Param('id') id: number) {
    return this.inventoryOrder.send({ cmd: 'MARK_PURCHASE_ORDER_AS_DEPARTED' }, id);
  }
  ...
  // @UseGuards(JwtGuard)
  // @Post('payment/customerPayment/receipt')
  // async saveCustomerPaymentReceipt(@Body() data: Data): Promise<any>{
  //   return this.paymantClient.send({ cmd: 'CREATE_CUSTOMER_PAYMENT_RECEIPT' }, data);
  // }
